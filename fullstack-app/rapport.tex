...
% Package pour les symboles mathématiques
\usepackage{amssymb}
...
\lstdefinelanguage{JavaScript}{
  keywords={break, case, catch, continue, debugger, default, delete, do, else, finally, for, function, if, in, instanceof, new, return, switch, this, throw, try, typeof, var, void, while, with, const, let, class, export, extends, import, super, yield, async, await, of, require, module, exports},
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
  morestring=[b]',
  morestring=[b]",
...
\begin{titlepage}
    ...
    \includegraphics[width=0.3\textwidth]{images/inpt_logo.png}\\[1cm]
    
    {\Large \textbf{Institut National des Postes et Télécommunications (INPT)}}\\[0.5cm]
    {\large Rabat, Maroc}\\[0.3cm]
...
\hline
stock & Number & Quantité en stock (entier $\geq$ 0) \\
\hline
...
\includegraphics[width=0.8\textwidth]{images/inscription.png}
...
\includegraphics[width=0.8\textwidth]{images/connexion.png}
...
\includegraphics[width=0.9\textwidth]{images/formulaire-creation.png}
...
\begin{itemize}
    ...
    \item Stock : Entier $\geq$ 0, obligatoire
...
\includegraphics[width=0.9\textwidth]{images/liste-produits.png}
...
\includegraphics[width=0.9\textwidth]{images/formulaire-modification.png}
...
La suppression nécessite une confirmation pour éviter les erreurs. Un dialogue de confirmation s'affiche avec les boutons Annuler et Confirmer avant toute suppression définitive.
...
\includegraphics[width=0.9\textwidth]{images/detail-produit.png}
...
\includegraphics[width=0.9\textwidth]{images/recherche-produits.png}
...
\includegraphics[width=0.9\textwidth]{images/filtrage-categorie.png}
...
\includegraphics[width=0.7\textwidth]{images/upload-image.png}
...
\includegraphics[width=0.9\textwidth]{images/dashboard-admin.png}
...
Un tableau affiche la répartition des produits par catégorie avec le nombre de produits et la valeur totale pour chaque catégorie.
...
\includegraphics[width=0.9\textwidth]{images/navbar-admin.png}
...
\includegraphics[width=0.9\textwidth]{images/accueil-visiteur.png}
...
L'API a été testée de manière exhaustive avec Postman. L'ensemble des 23 tests passés couvrent l'authentification, le CRUD produits, la pagination, le filtrage et la gestion des erreurs.
...
\begin{itemize}
    ...
    \item MONGODB\_URI=mongodb+srv://...@cluster0.mongodb.net/ecommerce
    \item JWT\_SECRET=[secret sécurisé]
    \item CLIENT\_URL=https://websec-bice.vercel.app
\end{itemize}
...
\textbf{Variables d'environnement :}
\begin{itemize}
    \item VITE\_API\_URL=https://redaelhadfii.onrender.com
\end{itemize}

\section{Application Déployée}

L'application est accessible en production aux adresses suivantes :
\begin{itemize}
    \item \textbf{Frontend (Vercel)} : \url{https://websec-bice.vercel.app/}
    \item \textbf{Backend API (Render)} : \url{https://redaelhadfii.onrender.com/}
    \item \textbf{Code source (GitHub)} : \url{https://github.com/redaelhadfi/websec/tree/main/fullstack-app}
\end{itemize}
...
\hline
T1 & Inscription avec email valide & $\checkmark$ Réussi \\
\hline
T2 & Inscription avec email existant & $\checkmark$ Erreur 409 \\
\hline
T3 & Connexion avec credentials valides & $\checkmark$ Réussi \\
\hline
T4 & Connexion avec mauvais mot de passe & $\checkmark$ Erreur 401 \\
\hline
T5 & Création produit par admin & $\checkmark$ Réussi \\
\hline
T6 & Création produit par user & $\checkmark$ Erreur 403 \\
\hline
T7 & Recherche de produits & $\checkmark$ Réussi \\
\hline
T8 & Filtrage par catégorie & $\checkmark$ Réussi \\
\hline
T9 & Upload image > 5MB & $\checkmark$ Erreur 413 \\
\hline
T10 & Pagination & $\checkmark$ Réussi \\
\hline
\end{tabularx}
...
\textbf{Test} & \textbf{Description} & \textbf{Résultat} \\
\hline
S1 & Accès route protégée sans token & $\checkmark$ Erreur 401 \\
\hline
S2 & Accès route admin par user & $\checkmark$ Erreur 403 \\
\hline
S3 & Token JWT expiré & $\checkmark$ Erreur 401 \\
\hline
S4 & Injection NoSQL dans login & $\checkmark$ Bloqué \\
\hline
S5 & Upload fichier .exe & $\checkmark$ Bloqué \\
\hline
S6 & XSS dans champ nom & $\checkmark$ Échappé \\
\hline
\end{tabularx}
...
\begin{itemize}
    \item $\checkmark$ \textbf{Authentification sécurisée} : JWT, bcrypt, expiration des tokens
    \item $\checkmark$ \textbf{Autorisation granulaire} : RBAC avec deux rôles distincts
    \item $\checkmark$ \textbf{CRUD complet} : Toutes les opérations implémentées
    \item $\checkmark$ \textbf{Recherche et filtrage} : Fonctionnalités avancées opérationnelles
    \item $\checkmark$ \textbf{Interface responsive} : Compatible mobile, tablette et desktop
    \item $\checkmark$ \textbf{Protection OWASP} : Injection, XSS, CSRF traités
    \item $\checkmark$ \textbf{Déploiement cloud} : Application accessible en production
    \item $\checkmark$ \textbf{Documentation complète} : 3768 lignes de documentation technique
\end{itemize}
...
gh repo create websec --public

# Push
git remote add origin https://github.com/redaelhadfi/websec.git
git branch -M main
...
NODE_ENV=production
MONGODB_URI=mongodb+srv://...@cluster0.mongodb.net/ecommerce
JWT_SECRET=your_secret_here
CLIENT_URL=https://websec-bice.vercel.app
\end{lstlisting}
...
#
# 4. Environment Variables:
VITE_API_URL=https://redaelhadfii.onrender.com
\end{lstlisting}
